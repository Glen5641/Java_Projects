...
public class SeriesInputWindow extends JFrame {
	 ...
	 */
	SeriesInputWindow() {
		setTitle("Add Episodes to Series");																							/*Title*/

		// Create panels for name, season number, episode number, and command
		// buttons
		JPanel jplTitle = new JPanel(new GridLayout(1, 0, 5, 5));													/*JPanelTitle   A Private Little War*/ 
		jplTitle.add(lblTitle);
		jplTitle.add(jtfTitle);

		JPanel jplSeasonNumber = new JPanel(new GridLayout(1, 0, 5, 5));								/*JPanelSeason Number 2*/
		jplSeasonNumber.add(lblSeasonNumber);
		jplSeasonNumber.add(jtfSeasonNumber);

		JPanel jplEpisodeNumber = new JPanel(new GridLayout(1, 0, 5, 5));								/*JPanelEpisodeNumber 19*/
		jplEpisodeNumber.add(lblEpisodeNumber);
		jplEpisodeNumber.add(jtfEpisodeNumber);

		JPanel jplYear = new JPanel(new GridLayout(1, 0, 5, 5));													/*JPanelYear 1968*/
		jplYear.add(lblYear);
		jplYear.add(jtfYear);

		JPanel jplButton = new JPanel(new GridLayout(1, 0, 5, 5));
		jplButton.add(jbtAddEpisode);																								/*JPanelButton AddEpisode*/
		jplButton.add(jbtClearInput);																									/*JPanelButton ClearInput*/
		jplButton.add(jbtClearEpisodes);																							/*JPanelButton ClearEpisodes*/
	...
	public void addClearEpisodesButtonListener(ActionListener clearEpisodesListener) {
		jbtClearEpisodes.addActionListener(clearEpisodesListener);
	}
