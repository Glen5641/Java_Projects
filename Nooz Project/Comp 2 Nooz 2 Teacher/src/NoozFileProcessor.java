 ...
 * into NewsStory and NewsMaker objects. Note that the field newsMakers and 
 * all methods in this class are static because we don't need to make several 
 * NoozFileProcessor objects and have them maintain their own lists of news 
 * makers. Instead, we simply need a collection of useful methods to create a 
 * list and return it to the calling method that will keep track of it.
 * 
 * @author Dean Hougen
 * @version 2.0
 * 
...
class NoozFileProcessor {
	...
	/**
	 * The primary method for reading the Nooz-style data file. It opens the file, 
	 * reads through it line by line until it reaches the end, and closes the file. 
	 * Each line is passed to processLine to parse and turn into data that is 
	 * actually added to the list. Note that this program doesn't attempt to deal 
	 * with I/O errors. This is allowable at this point to keep this project relatively 
	 * simple and because we haven't covered this topic yet. However, this is 
	 * something to be refined in the future.
	 * 
	 * @param fileName
	 ...
	 * and writes it to a file with the given file name. Note that this program 
	 * doesn't attempt to deal with I/O errors. This is allowable at this point 
	 * to keep this project relatively simple and because we haven't covered 
	 * this topic yet. However, this is something to be refined in the future.
	 * 
	 ...
	 * saving that data. This method breaks the line into parts, passes the 
	 * parts to appropriate helper methods that decode them and return data
	 * of appropriate types for the components of news stories and news 
	 * makers, puts these components together into a news story of the 
	 * proper type (newspaper story or TV news story) and two news makers, 
	 * and adds these to lists of news stories and news makers, respectively.
	 * 
	 * @param line
	 *            The line to process.
	 */
	private static void processLine(String line) {
		...
		String source = decodeSource(sourceCode);
		...
		if (sourceNum < 200) {
			newsStory = new NewspaperStory(date, source, wordCount, topic, newsMaker1, newsMaker2);
		}
		// Only two possibilities at this time
		else {
			newsStory = new TVNewsStory(date, source, wordCount, topic, newsMaker1, newsMaker2);
	 ...
	 * This method catches NumberFormatExceptions and deals with them locally 
	 * by printing messages to the error stream. In later assignments, we'll cover 
	 * better alternatives for handling exceptions.
	 * 
	 * @param dateString
	 *            The encoded date.
	 * @return The decoded date or null if the date cannot be
	 *         decoded.
	 */
	 ...
	 * the data file. Note that hard-coding the names is generally bad practice. It 
	 * would be better to keep the mapping from numeral to source name in a separate 
	 * file that could be read and used at runtime, so that the list of numerals and
	 * corresponding names could be updated without modifying the code. However,
	 * to keep this project relatively simple, we went with this "quick and dirty" approach.
	 * 
	 * @param sourceCode
	 ...
	 * string. This method catches NumberFormatExceptions and deals with
	 ...
	 * numeral in the data file. Note that hard-coding the topics is generally 
	 * bad practice. It would be better to keep the mapping from numeral to 
	 * topic in a separate file that could be read and used at runtime, so that 
	 * the list of numerals and corresponding topics could be updated without 
	 * modifying the code. However, to keep this project relatively simple, we 
	 * went with this "quick and dirty" approach.
	 * 
	 * @param topicCode
	 ...
	 * input line. Note that there are three distinctly different cases that need to be
	 * dealt with by this method. First, the special code "99" in the file corresponds 
	 * to the case when the story is not primarily focused on two named news makers. 
	 * In these cases, the code "99" may be used for one or both of the news maker 
	 * fields. We decode this as a news maker with the name "None" and allow the 
	 * user to explicitly or implicitly search for this news maker. Second, there may 
	 * be a news maker with a quoted name containing no commas. ("Obama 
	 * Administration" is one example.) In this case, the line parsed on commas will 
	 * include this news maker name in a single line part. Third, there may be a news 
	 * maker with a quoted name containing one comma. ("Romney, Mitt" is one 
	 * example.) In this case, the line parsed on commas will include this news maker 
	 * name in two line parts. These parts must be put together to create the news 
	 * maker name.
	 * 
	 * @param parts
